#!/usr/bin/env python3
"""Shape SLAT feature fusion on top of a chosen sparse fusion mode."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Multi-image sparse + SLAT fusion via run_multi_image"
    )
    ...
    p.add_argument(
        ...
        default="logit-mean",
        help="Occupancy fusion before SLAT (default: logit-mean)",
    )
    p.add_argument(
        "--mode",
        choices=["slat-mean", "slat-norm-weighted", "slat-max"],
        required=True,
        help="SLAT feature fusion mode",
    )
...
def main() -> int:
    ...
    if len(args.images) < 2:
        print("ERROR: slat_fusion requires at least 2 images.", file=sys.stderr)
        return 2

    strategy = "slat_fusion"
    run_dir = resolve_run_dir(args.output_dir, strategy, args.dest_dir)
    print_banner(
        f"{strategy} (sparse={args.sparse_mode}, slat={args.mode}) → {run_dir}"
    )
    ...
    try:
        ...
        with log_step("run_multi_image (sparse + SLAT fusion)") as t:
            torch.manual_seed(args.seed)
            out_multi, (shape_slat, _tex_slat, res_m) = pipeline.run_multi_image(
                raw,
                seed=args.seed,
                fusion_mode=args.sparse_mode,
        ...
        voxels_slat = int(shape_slat.coords.shape[0])
