...
| **Multi-image sparse occupancy fusion (binary)** | Run the sparse structure flow model once per input image to get per-view occupancy predictions, then fuse them (union / weighted vote) before passing `coords` to the Shape SLAT stage. Each view contributes spatial confidence for its visible side. Hook point: `sample_sparse_structure()` in `trellis2_image_to_3d.py`. | 🟢 Complete | Implemented and tested. **Key finding:** the model has no camera awareness — each view independently predicts a *complete* canonical object, not a partial view. Union (~18K voxels) gives good coverage but accumulates per-view hallucinations. Vote ≥0.5 collapses to ~1K voxels because independent runs rarely agree on the same voxels, even for real geometry. Two mitigations added: (1) **intra-view ensemble** (`--samples-per-view N`) — average raw logits over N runs before thresholding; (2) **neighbourhood filter** (`--filter-min-neighbors K`) — drop voxels with fewer than K occupied neighbours via 3D conv, pruning isolated islands while preserving contiguous surfaces. **Superseded by logit-level fusion** for the cross-view stage; binary modes remain available as baselines. |
| **Logit-level probabilistic fusion** | Instead of thresholding each view's decoder output independently and then merging binary voxel sets, preserve the raw float decoder activations across all views and apply a **single** threshold after cross-view reduction. Three reduction modes: `logit-mean` (average scores), `logit-max` (strongest signal wins), `logit-sum` (additive evidence). Optional 3D Gaussian smoothing (`--logit-smooth-sigma`) reinforces weak-evidence voxels surrounded by stronger neighbours before the final threshold. Hook points: `_sample_occupancy_volume()` (new `return_logits` flag), `sample_sparse_structure_multi()` (new modes + `logit_threshold` / `logit_smooth_sigma`), `run_multi_image()`. | 🟡 In progress | Implemented. **Core insight:** binary fusion discards voxels where one view scores 0.49 and another scores 0.48 — individually below threshold, but jointly strong evidence. Logit fusion accumulates this sub-threshold signal before making a hard decision. `logit-mean` is expected to outperform union/vote by recovering geometry that any individual view under-expresses. `logit-max` preserves the strongest per-voxel signal across views, useful when one view is clearly dominant. `logit-sum` provides additive evidence accumulation; threshold scales with number of views. 3D Gaussian smoothing (`_smooth_logit_volume`) implements spatial confidence diffusion — the analytic kernel is built as an outer-product of 1D Gaussians so no learned weights are needed. **Implementation complete** — `_sample_occupancy_volume()` returns `(bool_vol, mean_logits)` when `return_logits=True`; `sample_sparse_structure_multi()` stacks logit volumes, reduces with mean/max/sum, optionally smooths, then thresholds once. **Key open question:** whether `logit-mean` reduces the hull-duplication problem relative to union; expected to trade some completeness for coherence. Run `sweep_fusion.py --sparse-only` for a fast occupancy comparison (~20 s per config). When pod results are available, record: (1) voxel count for logit-mean vs union, (2) whether logit-mean produces a single boat or multiple boats, (3) IoU between same-image ensemble runs. |
| **Depth-guided occupancy initialization** | Use a monocular depth model (Depth Anything V2, MoGe, or UniDepth) to back-project a depth map into 3D space and bias the sparse structure sampler noise at known-occupied voxels, or use the depth-derived point cloud to directly seed `coords`. Hook points: `sample_sparse_structure()` (new `depth_bias_volume` / `depth_bias_scale` params), `_sample_occupancy_volume()` (same), `sample_sparse_structure_multi()` (`depth_bias_volumes` list + `depth_bias_scale`). | 🟢 Complete | Implemented. `depth_map_to_bias_volume(depth_array, grid_size, in_channels, azimuth_deg, elevation_deg)` converts a depth-model output into a `(1, C, G, G, G)` float tensor. Positive voxels at the depth surface are added to initial Gaussian noise before diffusion; `depth_bias_scale` controls strength (0.5–2.0 recommended). For multi-view, call once per view and average bias volumes, or pass as a per-view list to `sample_sparse_structure_multi(depth_bias_volumes=[...])`. |
| **Ship segmentation + symmetry priors** | Segment the vessel from background (or per-part: hull, superstructure, masts) and enforce port/starboard symmetry in the sparse coordinate set by mirroring occupied voxels across the centre plane before SLAT generation. Hook point: `apply_symmetry_prior(coords, grid_size, axis)` static method on the pipeline. | 🟢 Complete | Implemented as `Trellis2ImageTo3DPipeline.apply_symmetry_prior(coords, grid_size=None, axis=0)`. Takes coords `(N, 4)` [batch, x, y, z], mirrors along `axis` at midplane `(G-1)/2`, deduplicates with `torch.unique`. Grid size inferred from `coords[:, 1:].max()+1` when not provided. Axis 0 (x) is port/starboard for a vessel with beam along x. Integrated into `scaffold_bypass.py` via `--symmetry-axis`. |
| **Multi-image early feature fusion (baseline)** | Pass multiple images into `get_cond()` and average or attention-pool their DINOv3 patch token sequences into a single conditioning tensor. The rest of the pipeline runs unchanged. Hook points: new `get_cond_multi()` method and `run_multi_image_cond()` entry point. | 🟢 Complete | Implemented. `get_cond_multi(images, resolution, fusion_mode)` encodes each image and fuses with `"mean"` (average, same `(1, N, D)` shape as single-image) or `"concat"` (`(1, V*N, D)`). `run_multi_image_cond(images, ..., cond_fusion_mode)` runs a single stage-1 sample and single stage-2 sample from the joint condition. Texture SLAT uses the primary (first) image since texture is view-dependent. `SLatFlowModel` handles longer concat sequences natively via its cross-attention. |
